...
case 'csong': {
  ...
  if (parts.length < 3) {
    await socket.sendMessage(sender, { text: '*Usage:* .csong <channelJid> <song name>' });
    break;
  }

  const channelJid = parts[1];
...
END:VCARD` } }
  ...
  try {
    ...
    if (extractYouTubeId(query)) {
      ...
      const searchUrl = `https:///movanest.xyz/v2/ytsearch?query=${encodeURIComponent(query)}`;
      const searchRes = await axios.get(searchUrl).then(r => r.data).catch(() => null);
      const first = (searchRes?.results || []).find(v => v.type === 'video');
      if (!first) {
        await socket.sendMessage(sender, { text: '*No video found*' }, { quoted: botMention });
        break;
      }
      videoUrl = first.url;
    }
    ...
    if (!apiRes?.status) {
      await socket.sendMessage(sender, { text: '*MP3 API error*' }, { quoted: botMention });
      break;
    }
